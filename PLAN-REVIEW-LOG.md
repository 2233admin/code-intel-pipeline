...
Using read-only review evidence from `PLAN.md`, `run-code-intel.ps1`, `docs/artifact-data-contract.md`, local Sentrux shim.
...
- `PLAN.md` malformed around approach items 7-9; conflict-schema consumer-migration instructions truncated into fragments. Fix: rewrite items 7-9 complete sentences exact required fields exact functions/outputs update.
- `PLAN.md` invalid/incomplete JSON in Minimal Artifact Shape, dropping fields around `gate.target`, `conflicts`, `parser`. Fix: replace sample valid JSON parsed `ConvertFrom-Json`.
...
- Rewrote `PLAN.md` completely to remove malformed fragments.
...
- `PLAN.md` still malformed approach items: item 7 ends mid-sentence, item 8 fragment, items 10-11 empty. Fix: rewrite approach executable steps complete required fields, consumers, tests, outputs.
- `PLAN.md` still contains invalid JSON despite line claiming `ConvertFrom-Json`; `source` no value, `before/after/target/conflicts/parser` keys missing collapsed, line contains raw text. Fix: replace sample syntactically valid JSON include full `gate`, `conflicts`, `parser`, raw-source-pointer fields.
- `PLAN.md` says conflict records need ids, metrics, values, source names, but prior raw source pointer requirement not actually specified in artifact shape. Fix: add `source_step`, `raw_output_path`, bounded `stdout_excerpt`, `parsed_at` to failures/conflicts.
- `PLAN.md` mentions GitHub failed-step non-authoritative but not whether `sentrux-failures.json` is passed to GitHub research or excluded by design. Fix: explicitly state GitHub research consumes normalized failures context and raw-step-only cannot select primary targets.
...
- Rewrote `PLAN.md` with a direct PowerShell here-string because previous patch attempts left malformed markdown/JSON fragments.
...
- `PLAN.md` approach items now name discovery surfaces, consumers, GitHub handling, parser tests, integration assertion, Ponytail gating.
...
- `PLAN-REVIEW-LOG.md` records Round 3 response claimed fixes.
