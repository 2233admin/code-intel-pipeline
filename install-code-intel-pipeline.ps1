...
function Invoke-SentruxInstall {
    ...
    throw "no automatic sentrux installer found; install Rust/cargo first or put sentrux on PATH"
}
...
function Get-InstallMetadata {
    ...
    switch ($CommandName) {
        ...
        "sentrux" { return [ordered]@{ packageManager = "cargo"; requiresElevation = $false } }
    }
...
function Install-SentruxShim {
    ...
    try {
        ...
        Add-InstallAction $Actions "sentrux-shim" "installed" "shim=$shimDir path=$($pathResult.detail)" "Open a new terminal if this shell cannot find sentrux from PATH." "repo-local" $false
    }
    catch {
...
Add-InstallPlan $installPlan "sentrux" "cargo" "cargo install sentrux --locked" "Structural quality and regression gate." "MEDIUM: cargo source must be trusted; no automatic install if cargo is absent." "The repo-owned sentrux-lite core keeps scan/check/gate usable until the real binary is installed." "cargo" $false
...
Install-MissingTool $installActions "sentrux" { Invoke-SentruxInstall } "Install sentrux or ensure it is on PATH."
