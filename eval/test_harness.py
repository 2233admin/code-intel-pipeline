...
class GoldenAnchorTests(unittest.TestCase):
    ...
    cannot silently rot the benchmark (issue #93 golden-symbol-anchors;
    proof case: PR #134 shifted crates/code-intel-cli/src/sentrux_gate.rs's
    evaluate_rules from line 481 to line 537 without changing what it does,
    which alone dropped the frozen "how" baseline from 5/5 to 3/5)."""
...
class QuestionSchemaTests(unittest.TestCase):
    def test_real_questions_file_has_twelve_valid_entries(self):
        ...
        for q in questions:
            ...
            for span in q["golden"]["spans"]:
                # eval/questions.json is fully migrated to content anchors
                # (issue #93 golden-symbol-anchors): every real span must
                # carry an 'anchor', not an absolute start_line/end_line.
    ...
    def test_real_questions_anchors_all_resolve_against_the_live_tree(self):
        ...
        # (see GoldenAnchorTests / eval/golden_anchors.py). q01 and q02 are
        # the one known, expected exception: PR #134 moved
        # is_primary_invocation and the RAW_ROUTES table out of main.rs
        # entirely, which is a real code move this resolver correctly
        # refuses to paper over -- it must report exactly those two broken,
        # and nothing else.
