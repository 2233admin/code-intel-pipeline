...
# PowerShell entry points are $RepoRoot-relative (archive/); orchestration/
# stayed at the repository root one level above it.
...
function Get-TextSha([string]$Text) { ([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData([Text.Encoding]::UTF8.GetBytes($Text)))).ToLowerInvariant() }
...
# Staleness. This must mirror New-ProviderPreflightRetirementPacket.ps1's
# $snapshotIdentity composition exactly: the live facade hash, the SHA-256 of
# the historical branch text, and the two live inputs. E03 previously had no
