...
$repoRoot = $root
$facade = Get-Content -Raw -LiteralPath (Join-Path $root "legacy/run-code-intel.ps1")
...
$probe = Join-Path $root "legacy/Invoke-RepowiseProviderProbe.ps1"
try {
...
try {
    ...
    $facadeOutput = & pwsh -NoProfile -File (Join-Path $root "legacy/run-code-intel.ps1") `
