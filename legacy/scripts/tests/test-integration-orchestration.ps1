...
if ($mergeQueue[0].entrypoint -ne "legacy/Invoke-MultiAgentMergeQueue.ps1" -or @($mergeQueue[0].capabilities).Count -lt 4) {
...
if ($canonicalCodeNexus[0].commandTemplate -ne 'pwsh -NoProfile -File "$env:CODE_INTEL_HOME\legacy/Invoke-CodeNexusLite.ps1" -RepoPath ''<repo-path>''') {
...
if ($providerPlan.command -notmatch '^pwsh -NoProfile -File "\$env:CODE_INTEL_HOME\\legacy/Invoke-CodeNexusLite\.ps1" -RepoPath ''.+code intel provider plan''$') {
...
$parityBaseline = Join-Path $root "scripts/tests/test-parity-baseline.ps1"
