...
try {
    ...
    $result = & (Join-Path $root "legacy/update-code-intel-index.ps1") -ArtifactRoot $temp -OutputPath $output | ConvertFrom-Json
    ...
    $legacy = & (Join-Path $root "legacy/update-code-intel-index.ps1") -ArtifactRoot $temp -OutputPath $legacyOutput -LegacyCompatibilityMode | ConvertFrom-Json
    ...
    $script = Get-Content -LiteralPath (Join-Path $root "legacy/update-code-intel-index.ps1") -Raw
