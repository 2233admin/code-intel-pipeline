...
$delegate = Join-Path $repoRoot "legacy/Invoke-ModelChannelDelegate.ps1"
...
try {
    ...
    & pwsh -NoProfile -File (Join-Path $repoRoot "legacy/run-code-intel.ps1") -ModelAdapterRequest $deniedRequest -ModelAdapterArtifactRoot $facadeArtifacts 2>&1 | Out-Null
