...
try {
    ...
    # the recovery launcher ships under legacy/ from 0.7.0-beta.2 onward
    $invokeWrapper = Join-Path $packageRoot "legacy/code-intel.ps1"
    Assert-Condition (Test-Path -LiteralPath $invokeWrapper -PathType Leaf) "Package is missing legacy/code-intel.ps1."
