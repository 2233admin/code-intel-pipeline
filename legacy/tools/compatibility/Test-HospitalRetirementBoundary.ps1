...
if($functionBlocks-ne1-or$callBlocks-ne1){throw "E08 requires exactly one embedded Hospital function block and one production call block"};if($integration.Count-ne1){throw "B09 registry declaration is absent or ambiguous"}
[ordered]@{schema="code-intel-hospital-retirement-boundary.v1";ok=$true;branchId="run-code-intel.hospital.embedded-diagnosis-render";affectedFiles=@("run-code-intel.ps1");embeddedFunctionBlocks=$functionBlocks;embeddedCallBlocks=$callBlocks;b09RegistryRoutes=$integration.Count;normalFacadeUsesB09=$normalB09Route;legacyRemainsAuthority=(-not$normalB09Route);publicationTouched=$false;indexTouched=$false;doctorTouched=$false;deletionExecuted=$false;retired=$false}|ConvertTo-Json -Compress
