...
<p align="center">
  <img src="assets/gpt-musume.svg" alt="GPT娘正在给代码仓库做结构检查" width="760">
</p>

<p align="center">
  <b>把一个刚从 GitHub 拉下来的项目，交给 GPT 娘做体检、拍片、诊断、开手术单。</b>
</p>

<p align="center">
  <code>rg</code> + <code>Repowise</code> + <code>Understand Anything</code> + <code>Sentrux</code> + <code>CodeNexus-lite</code>
  <br>
  一个面向 AI Agent 的本地代码理解流水线。
</p>
...
`Code Intel Pipeline` 是一套本地仓库理解工具链。它不是又一个聊天机器人外壳，也不是把代码塞进向量库就假装懂了。

它做的事情更朴素，也更有用：

1. 你把一个 GitHub 项目拉下来。
2. 它扫描文件、结构、依赖、复杂度、测试缺口和治理规则。
3. 它生成一组机器可读和人可读的报告。
4. Agent 在改代码前后都能用同一套结构信号复查，避免高速制造结构债。
...
GPT 娘负责接诊。她不替你吹项目，她只问一句：这个仓库现在能不能出院？
...
GPT 娘小纸条：别让 Agent 裸奔。没有 `session_start/session_end`，它改完代码以后自己也不知道有没有把结构弄坏。
...
先跑安装器，再跑 doctor，再跑 normal。读 `summary.md`，失败看 `report.json`，交接看 `understanding.md`，治理看 `hospital.md`，动刀看 `surgery-plan.md`。不要跳过 Sentrux baseline 和 rules，否则你不是在协作，你是在让机器以光速堆债。
