...
| `-SkipRepowise` (switch) | `:71`, gate at `:3646` | 5 files (`test-code-evidence-layer.ps1`, `test-code-intel-pipeline.ps1`, `test-primary-launchers.ps1`, `test-runtime-ci-hospital-pet.ps1`, `test-transactional-publication.ps1`) plus `tests/stable_wrapper_e2e.rs`, which asserts the skip reaches the authoritative doctor policy | port | Skips the entire Repowise stage. |
