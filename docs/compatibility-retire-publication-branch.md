...
E05 owns only the two bounded legacy publication hunks in `archive/run-code-intel.ps1`: staging-directory
...
```powershell
pwsh -NoProfile -File archive/tools/compatibility/Test-PublicationRetirementBoundary.ps1
pwsh -NoProfile -File archive/tools/compatibility/New-PublicationRetirementPacket.ps1 `
...
pwsh -NoProfile -File archive/tools/compatibility/Test-PublicationRetirementPacket.ps1 `
...
`retired=false`. `archive/update-code-intel-index.ps1` is neither read as deletion input nor modified by E05.
