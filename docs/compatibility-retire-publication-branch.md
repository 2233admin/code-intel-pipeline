...
E05 owns only the two bounded legacy publication hunks in `legacy/run-code-intel.ps1`: staging-directory
...
```powershell
pwsh -NoProfile -File legacy/tools/compatibility/Test-PublicationRetirementBoundary.ps1
pwsh -NoProfile -File legacy/tools/compatibility/New-PublicationRetirementPacket.ps1 `
...
pwsh -NoProfile -File legacy/tools/compatibility/Test-PublicationRetirementPacket.ps1 `
...
`retired=false`. `legacy/update-code-intel-index.ps1` is neither read as deletion input nor modified by E05.
