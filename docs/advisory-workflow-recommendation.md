...
`archive/OpenSpec-Detector.ps1` is the standalone atom. `archive/Invoke-WorkflowRecommendation.ps1` is the compatibility facade wrapped by the A01 runtime adapter; `archive/run-code-intel.ps1` invokes the A01 capability. Historical `-SkipOpenSpec` disables the advisory call and `-AutoOpenSpec` maps to the facade's recorded `-Auto` compatibility option; neither flag grants adoption authority.
...
OpenSpec OPSX, spec-kit, gstack, and matt-flow are candidates, not runtime dependencies. The rollback path is direct invocation of `archive/OpenSpec-Detector.ps1`.
