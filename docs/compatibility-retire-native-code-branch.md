...
`New-CodeEvidenceLayer` call in `legacy/run-code-intel.ps1`. It does not own the B08 algorithm, A09 DAG,
...
```powershell
pwsh -NoProfile -File legacy/tools/compatibility/New-NativeCodeRetirementPacket.ps1 `
  -OutDir orchestration/retirements/e07-native-code `
  -EvaluatedAt <unix-seconds>
pwsh -NoProfile -File legacy/tools/compatibility/Test-NativeCodeRetirementPacket.ps1 `
