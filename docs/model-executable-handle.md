...
`legacy/New-ModelAdapterRequest.ps1` converts a ready routing result and the matching inventory candidate into a closed v2 adapter request. Cost scope, model identity, external-egress posture, and consent states are derived from those inputs; a caller cannot weaken them in a hand-authored synthesis request.

CLI requests use `legacy/New-ModelExecutableHandle.ps1`. The short-lived handle binds adapter identity, canonical path, SHA-256, file length, last-write ticks, observation time, and expiry. `legacy/Invoke-ModelChannelDelegate.ps1` revalidates every field immediately before starting the process and rejects expired, mutated, moved, or adapter-mismatched executables before invocation.
