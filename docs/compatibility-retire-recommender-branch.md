...
E02 is limited to the duplicated inline workflow recommender in `legacy/run-code-intel.ps1` and its one
...
```powershell
pwsh -NoProfile -File legacy/tools/compatibility/New-RecommenderRetirementPacket.ps1 `
...
approval subject binds `legacy/run-code-intel.ps1::run-code-intel.workflow-recommender.inline` and the
...
```powershell
pwsh -NoProfile -File legacy/tools/compatibility/Restore-RecommenderLegacyBranch.ps1 `
