...
## Abstract
Provide one language-neutral acceptance entry point for agent, landing, and promotion decisions. Reuse `legacy/scripts/tests/Test-CodeIntelProjectConformance.ps1` and its policy so each stage selects an existing project profile instead of copying suite logic.
...
## Success Criteria
- [x] `legacy/Invoke-CodeIntelAcceptance.ps1 -Stage agent|land|promote` maps stages to targeted+fast, fast, and full respectively.
- [x] The entry point delegates project suites to `legacy/scripts/tests/Test-CodeIntelProjectConformance.ps1` without duplicating suite implementations.
...
- Start with `legacy/install-code-intel-pipeline.ps1 -RepoPath <repo>`, then doctor/lite/normal as required by the Code Intel skill.
