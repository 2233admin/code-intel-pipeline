...
- **Dependencies:** none.
- **Affected files (initial):** `tests/fixtures/parity/**` (new), `legacy/scripts/tests/test-code-intel-pipeline.ps1`, `legacy/scripts/tests/test-integration-orchestration.ps1`.
...
- **Compatibility / rollback:** `legacy/run-code-intel.ps1` remains the outer facade and may select legacy sequential orchestration; rollback switches the facade route without deleting coordinator state.
...
- **Dependencies:** A07.
- **Affected files (initial):** index reader/writer in Rust, `legacy/update-code-intel-index.ps1` adapter, tests.
...
- **Dependencies:** A04.
- **Affected files (initial):** `crates/code-intel-cli/src/providers.rs`, Repowise adapter, `legacy/scripts/tests/test-code-intel-provider.ps1`, conformance fixtures, facade route.
...
- **Dependencies:** A04.
- **Affected files (initial):** `legacy/Invoke-SentruxAgentTool.ps1`, Sentrux adapter/schema/fixtures, normalization tests, integration registry.
...
- **Dependencies:** A04.
- **Affected files (initial):** `orchestration/integrations.json`, CodeNexus port fixtures/adapter, `legacy/Invoke-CodeNexusLite.ps1`, `crates/code-intel-cli/src/providers.rs`.
...
- **Affected files (initial):** `legacy/OpenSpec-Detector.ps1`, new atom implementation/contract, `legacy/run-code-intel.ps1`, `legacy/scripts/tests/test-workflow-recommendation-brief.ps1`.
...
- **Affected files (initial):** `orchestration/integrations.json`, `legacy/run-code-intel.ps1`, `legacy/Invoke-CodeIntelOrchestrator.ps1`, registry audit test.
...
- **Affected files (initial):** native code-evidence functions currently in `legacy/run-code-intel.ps1`, new atom module/adapter, Code Evidence schemas, A/B and contract tests, registry declaration.
...
- **Affected files (initial):** `New-Hospital*`/`Get-Hospital*` functions currently in `legacy/run-code-intel.ps1`, new diagnosis atom, hospital JSON schema/fixtures, rendering separation tests, registry declaration.
...
- **Affected files (initial):** `legacy/check-code-intel-tools.ps1`, stable wrapper doctor path, Rust `doctor` command/adapter, doctor schema/fixtures, registry/manifest tests.
...
- **Compatibility / rollback:** `legacy/check-code-intel-tools.ps1` remains the bootstrap adapter until E09 approval; rollback restores wrapper routing without changing the doctor contract.
...
- **Smallest proving test:** reject a record citing only `legacy/scripts/tests/test-skill-development-benchmark.ps1` pass; require pinned source and semantic conformance.
...
- **Owner / boundary:** executor; owns deletion of the duplicated workflow-recommender branch in `legacy/run-code-intel.ps1`, not recommender behavior.
- **Dependencies:** E01, B06.
- **Affected files (initial):** `legacy/run-code-intel.ps1`, `legacy/OpenSpec-Detector.ps1` adapter path, recommender parity tests, one retirement record.
...
- **Owner / boundary:** executor; owns removal of direct production invocation of `legacy/scripts/tests/test-code-intel-provider.ps1`, not Repowise behavior.
- **Dependencies:** E01, B01.
- **Affected files (initial):** `legacy/run-code-intel.ps1`, Repowise facade route, provider tests, one retirement record.
...
- **Affected files (initial):** `legacy/run-code-intel.ps1`, CodeNexus adapter route, localization/survival tests, one retirement record.
...
- **Affected files (initial):** `legacy/run-code-intel.ps1`, B08 adapter route, code-evidence tests, one retirement record.
...
- **Affected files (initial):** `legacy/run-code-intel.ps1`, B09 adapter route, hospital parity/precedence/rendering tests, one retirement record.
...
- **Affected files (initial):** `legacy/invoke-code-intel.ps1`, `legacy/check-code-intel-tools.ps1`, Rust doctor adapter route, doctor tests, one retirement record.
