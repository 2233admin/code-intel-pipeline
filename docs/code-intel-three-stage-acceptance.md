...
`legacy/Invoke-CodeIntelAcceptance.ps1` is the language-neutral acceptance entry point used before an
...
Project suites remain owned by `legacy/scripts/tests/Test-CodeIntelProjectConformance.ps1` and
...
$target = @{
    ...
    kind = "pwsh"
    file = "legacy/scripts/tests/test-code-intel-acceptance.ps1"
    args = @()
} | ConvertTo-Json -Compress

./legacy/Invoke-CodeIntelAcceptance.ps1 -Stage agent -TargetCheckJson $target -Json
```
...
```powershell
./legacy/Invoke-CodeIntelAcceptance.ps1 -Stage land -Json
./legacy/Invoke-CodeIntelAcceptance.ps1 -Stage promote -Json
```
