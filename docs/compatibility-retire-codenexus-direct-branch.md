...
E04 is restricted to the one normal-run branch in `archive/run-code-intel.ps1` that directly invokes
`archive/Invoke-CodeNexusLite.ps1`. It does not authorize changes to the lite implementation, B04 provider
...
blocked packet and leaves `archive/run-code-intel.ps1` unchanged.

Generate and validate a fresh packet after building the Rust CLI:

```powershell
pwsh -NoProfile -File archive/tools/compatibility/New-CodeNexusDirectRetirementPacket.ps1 `
  -OutDir orchestration/retirements/e04-codenexus-direct `
  -EvaluatedAt <unix-seconds>
pwsh -NoProfile -File archive/tools/compatibility/Test-CodeNexusDirectRetirementPacket.ps1 `
