...
only implementation of Pipeline execution semantics. `legacy/code-intel.ps1` is a
...
`code-intel .` and `legacy/code-intel.ps1 .` analyze the given Target Repository, or
the current directory when omitted, using `normal` mode by default. The existing
`legacy/invoke-code-intel.ps1` remains a quiet forwarding compatibility surface
...
- Documentation and Skills lead with `code-intel`; they mention
  `legacy/code-intel.ps1` only for installation and recovery.
