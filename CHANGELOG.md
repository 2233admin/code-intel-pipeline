...
<<<<<<< HEAD
- **`code-intel edit apply-plan` + `edit.ast-grep-apply` 能力：ast-grep 计划真实执行，整份计划要么全落要么全不落**（#96 item 2、charter gate G4 #139）。`edit.ast-grep-plan` 现在除预览外还产出一份**指令**：`apply.edits[]` 给每个匹配的 span（1-based 行列、结束列开区间，与 `edit apply` 同一套坐标）、这些字节当时的 sha256、以及 ast-grep 算出的替换文本；能带 `replacementOffsets` 时按更窄的那个范围记，改一个标识符就只动那个标识符。每个匹配记两个 digest：span 自身的字节，以及它所在那一行——狗粮时发现光记 span 有洞，手工把 `commit` 改成 `commit_now` 后 span 那六个字节还是 `commit`，照写就成了 `commit_change_now`；匹配本来就是那一行的 parse 产物，行变了计划就不该再落，`line_drift` 即为此。`edit apply-plan --plan <structured-edit-plan.json>` 执行它：**写第一个字节之前**先把每个文件里每个 span 的两个 digest 全部核对一遍，任何一个漂移就整份拒绝（退出码 10、`applied:false`），并点名是哪几个匹配漂了（`expectedSha256` / `foundSha256` / 有界原文）——没有部分应用，拒绝不会留下改了一半的树。核对通过后先给所有文件写同级临时文件，全部落盘才逐个 rename；产生新字节阶段失败即整体回滚，rename 阶段失败则如实点名哪些文件已经换掉、digest 是多少。模型只发计划，字节变换全部由工具执行——没有任何一条路径能让调用方递进重写后的文件内容；计划自带的 `text` 必须能哈希出它自己声称的 `sha256`，对不上直接拒。写路径同样走既有 capability envelope（`edit.ast-grep-apply.compat`，`allowedEffects` 含 `repo_mutation`，动手前先查请求的 effectPolicy），span 寻址算术、路径策略与原子写三者由 `span_patch` 一处提供，计划面与两个应用面共用而非各自复述。计划不与自己的 snapshot 绑死（否则 scope 内任何无关改动都会作废一份 span 完好的计划，digest 也就成了摆设）：snapshot 现算，per-span digest 才是闸门。
...
=======
- **`code-intel edit apply-plan` + `edit.ast-grep-apply` 能力：ast-grep 计划真实执行，整份计划要么全落要么全不落**（#96 item 2、charter gate G4 #139）。`edit.ast-grep-plan` 现在除预览外还产出一份**指令**：`apply.edits[]` 给每个匹配的 span（1-based 行列、结束列开区间，与 `edit apply` 同一套坐标）、这些字节当时的 sha256、以及 ast-grep 算出的替换文本；能带 `replacementOffsets` 时按更窄的那个范围记，改一个标识符就只动那个标识符。每个匹配记两个 digest：span 自身的字节，以及它所在那一行——狗粮时发现光记 span 有洞，手工把 `commit` 改成 `commit_now` 后 span 那六个字节还是 `commit`，照写就成了 `commit_change_now`；匹配本来就是那一行的 parse 产物，行变了计划就不该再落，`line_drift` 即为此。`edit apply-plan --plan <structured-edit-plan.json>` 执行它：**写第一个字节之前**先把每个文件里每个 span 的两个 digest 全部核对一遍，任何一个漂移就整份拒绝（退出码 10、`applied:false`），并点名是哪几个匹配漂了（`expectedSha256` / `foundSha256` / 有界原文）——没有部分应用，拒绝不会留下改了一半的树。核对通过后先给所有文件写同级临时文件，全部落盘才逐个 rename；产生新字节阶段失败即整体回滚，rename 阶段失败则如实点名哪些文件已经换掉、digest 是多少。模型只发计划，字节变换全部由工具执行。这份保证的边界值得说清：`--plan` 就是磁盘上的一份 JSON，`schema` / `capability` / `snapshotIdentity` 都是作者自己写的字符串，本阶段**不声称**、也无从证明一份计划来自真实的 `edit.ast-grep-plan` 运行（能手写计划的调用方，同样能直接跑计划能力）。真正被强制的是一条更窄、可机械检验的性质：**只能写到自己已经读过的地方**——计划里的 `text` 必须能哈希出它声称的 `sha256`，落盘时该 span 现在的字节也必须哈希成同一个值，其所在行还要哈希成 `lineSha256`；每条替换再受与计划阶段同一个 4096 字节上限约束，所以手写计划塞不进一整份文件的新内容。`snapshotIdentity` 只作溯源上报，不作信任依据。写路径同样走既有 capability envelope（`edit.ast-grep-apply.compat`，`allowedEffects` 含 `repo_mutation`，动手前先查请求的 effectPolicy），span 寻址算术、路径策略与原子写三者由 `span_patch` 一处提供，计划面与两个应用面共用而非各自复述。计划不与自己的 snapshot 绑死（否则 scope 内任何无关改动都会作废一份 span 完好的计划，digest 也就成了摆设）：snapshot 现算，per-span digest 才是闸门。
>>>>>>> ff9e97b (fix(edit): 把"没有任何一条路径能递进重写后的文件内容"改成真话，并给它补上真正的边界 (#96 #139))
