...
- **安装器从未把 `legacy/run-code-intel.ps1` 与 `pipeline.config.json` 部署进 `<bin>`，导致每个正式发布安装都过不了 doctor bootstrap 就绪检查（#232）**：`doctor_bootstrap::observe` 无条件要求这两个文件存在于 `<bin>` 下，跟 `require_understand`/`require_repowise` 不同，没有 `options` 开关；而 `Install-CodeIntelBinary` 只部署了编译产物和 `orchestration/integrations.json`。新增 `Install-LegacyPipelineEntrypoint`（复刻既有 `Install-IntegrationsManifest` 的 `Copy-Item` 写法）补齐这一步，并按 DR-0001 把复现测试并入 install-smoke CI 闸（非 ignore 断言部署步骤存在，`#[ignore]` 版由打包安装 CI 步骤实跑）。
- **`unique_temp_dir` 跨进程/并发路径碰撞（#352）分三批修完**：`TempReport::write`、`mcp_serve::staging_path`、`project_context::run` 三处生产/测试路径改用 pid + `AtomicU64` 序号（`TempReport` 额外带 `module_path!()` owner token，防 `#[path]` 重挂载）（#353）；`session_evidence::write_new_artifact`（四处里最后一处，此前靠 `create_new(true)` 只是报错而非静默覆盖，仍是真 bug）同法修复（#354）；随后把仓内 7 处独立重实现的 `unique_temp_dir`-类测试 helper 收拢进新增的共享 `test_support::unique_temp_dir`（#231 提案 Option B，小型共享 helper），`graph/tests.rs` 因仍是 `#[path]` 挂载进 12 个集成测试二进制而保留独立实现不纳入本次收拢（#356）。
- **`perf_optimize::weco_process` 超时测试在 CPU/构建争用下抖动（#349）**：四处独立根因一次修完——Windows fixture 用 `ping -n 1 -w 20` 轮询停止标记，`-w` 只界定等回复的上限而非保证延时，回复通常 <1ms，实测循环变成自发的 ping.exe 进程风暴反过来制造争用；改用 `ping -n 2`（无 `-w`）借其约 1.1s 默认间隔。测试曾用 `graceful_stop_invoked`（"停止命令已发出"）当"优雅退出"的代理断言，现改为显式 `hard_killed` 字段直接断言。超时阈值按证据放宽（`wall_clock_timeout` 50ms→2s，Windows `grace_period` 3s）。决策逻辑抽成 `resolve_after_timeout`（`TimeoutChild` trait + mock），零真实进程/时钟/sleep 即可确定性验证，并发全量套件下 20/20 稳定。
- **`sentrux-hotspots.json` 唯一产出方是 `legacy/run-code-intel.ps1` 的 `Select-Object -First 30/50/20`，任意 top-N 截断静默丢弃阈值以下文件，`session-adapt --hotspots` 对只碰低复杂度文件的 session 零匹配却无任何降级信号（#361）**：新增 Rust 原生 producer `sentrux_hotspots.rs` 保留全部行不截断，facade 唯一路径切到该 producer 且无 PS1 回退（任何回退都会原样复现被修的截断 bug，fail closed）（#362）。修完后发现该 fail-closed 改写与 dsm/file-details/evolution/what-if 共用外层 try/catch，hotspots 未捕获的 throw 会连带跳过 evolution/what-if 这两个本身工作正常、有独立 fallback 的阶段；补上 hotspots 专属 try/catch 隔离失败面（#365/#366）。
- **`code-intel verify` 的参数解析静默吞掉未识别的 dash 前缀参数，`verify . --write` 退出码 0 如同已接受（#369）**：`run_raw` 现在把每个原始参数三分类（`--json` / 位置参数候选 / 未识别 dash 前缀标志），恰好一个位置参数才合法，未识别标志一律走既有 `report_usage_error` 拒绝，退出码 64。
- **`sentrux.dsm` 的模块耦合矩阵对单 crate/PowerShell 为主的仓库结构性为空（#376，收敛 #148 E1）**：`dsm_edges` 此前只解析 `.py`/`.rs`/`.v`，且 `module_name()` 把 `crates/<name>/src/**` 下所有文件桶进同一个 `crates/<name>` 模块——`dsm_edges` 按构造排除同模块边，本仓自身的 Rust 耦合因此永远测不出来。现按 `crates|packages/<name>/src|app|tests/<file-or-subdir>` 补一级细粒度（同 `backend/<app|src|tests>/<next>` 既有分桶手法），并新增 PowerShell dot-source（`. (Join-Path $PSScriptRoot "x.ps1")`）与 `Import-Module`（含先赋值变量再引用的两段式）解析，覆盖本仓真实使用的两种依赖写法。`sentrux.dsm` 的 `note` 字段同时改写，去掉承诺一个生产环境实际不可达的外部 Sentrux/CodeNexus 工具的措辞（DR-0010），`currentState` 提升为 `authoritative_automatic`。#148 的 E1（唯一剩余未关项）随本修复关闭，C2 此前已由 #152 修复。落地时 `sentrux-capability-gate` CI 实测揭出 `MAX_COMMAND_EVIDENCE_BYTES`（自 #286 起从未复议的 1MiB 上限）已过时——本仓 `sentrux.dsm` 真实诚实输出现约 3.8MB，超限即被判 `degraded` 拖累新拿到的 `authoritative_automatic` 断言，遂放宽至 16MiB（DR-0010 附录）。验证过程中同时发现一个更深、与本次改动无关的既存缺陷：`outputs.structuredData` 依赖另一个更小的 8KB 预览截断，`sentrux.scan` 早已同样受影响（`status: succeeded` 但 `structuredData: null`），另开 #383 追踪，不在本次改动范围内解决。
...
- **session-observability 原型经真实试跑后判定不达晋升门槛，移除**（#359/#360）：安装 cosmtrek/mindwalk（钉版本），用真实 Claude Code session 生成的 trace 对照该 session 提交前后状态生成的规范 `sentrux-hotspots.json` 实测联接——5 条晋升标准中"trace 目标能联接上 Sentrux hotspot 记录"实测 0 联接（session 只碰了 2 个小测试文件，均不在旧 top-30 复杂度截断内），未达 IDEA.md 定的晋升门槛（"仅当真实 trace 产出有用的路径联接且 timeline 暴露 review 相关信号才晋升"），按负结果移除原型，issue 记录结论与成因供后续复议。
- **`sentrux_gate` 的 `quality_signal` 从 `10000 - penalty` 代理分数改为上游兼容的 Quality Signal，并输出五个根因（#385，父级 #285）**：新增 `sentrux_quality_signal.rs` 计算内核，按上游文档（https://sentrux.dev/docs/quality-signal/）与固定源码版本（`6f8ff3c14b0423e4b58f42d1813d4d5f7fdc1d11`）移植 modularity（Newman Q）、acyclicity、depth、equality（Gini）、redundancy 五因子归一化公式与几何平均聚合，显式记录并保留固定源码相对文档页多出的 `max(0.01)` 逐因子下限（`FORMULA_VERSION`）。原生扫描器此前无跨语言解析的文件依赖图，`measure_project` 新增有界启发式导入目标解析（引号相对路径 + 唯一文件名回退）喂给 modularity/acyclicity/depth；equality 采用上游自身定义的"无函数级 CC 数据时退回文件行数"回退路径（本仓从未追踪过函数级复杂度）；redundancy 目前只实现 duplicate 一半（跨文件规范化内容比对），dead-function 检测未实现——本仓 `cargo check` 的 ~100 条 dead-code 警告本就是 `#[path]` 重复编译的编译器产物而非真实死代码信号，朴素的"从未被调用"启发式对本仓会产生大量假阳性，因此诚实标记为 `completeness: "partial"` 并在 `note` 字段说明，不伪造 `dead_count = 0`。`scan`/`health` 新增 `quality_signal_detail`（`schema`/`formula_version`/`provider_version`/`bottleneck`/`completeness`/`root_causes.<metric>.{score,raw,completeness}`）。旧代理分数与新分数语义不兼容，`BASELINE_SCHEMA` 随之升到 v6（`ENGINE_VERSION` 3.0.0），旧 v5 基线按既有 `baseline_engine_mismatch` 机制拒绝直接比较并给出机器可读原因，避免伪造 before/after；本仓自身 `.sentrux/baseline.json` 已用新引擎重新保存（非回归抑制，是既有"改变多 session 行为口径必须重新定基线"惯例的直接后果）。见 DR-0011。
