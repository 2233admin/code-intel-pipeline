...
- [ ] 6.4 Push a review-fix checkpoint and observe the hosted rerun. Stop at the real `agent-approved`/human-merge boundary; do not self-approve, bypass, archive, merge, or begin outcome observation. Blocked on hosted run `31311850197`: all three platform smoke jobs pass, but the Windows main job still fails the unrelated frozen E02/E03/E04/E07/E08 retirement-packet attestations.
- [x] 6.5 Reproduce and close the hosted macOS hermetic-test regression by routing the compatibility-facade binary override through shared test support; rerun the affected local and hosted contracts.
