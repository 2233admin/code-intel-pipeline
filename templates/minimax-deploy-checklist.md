...
目标：在一台新机器上，把 Code Intel Pipeline 跑到可用状态。
...
.\legacy\bootstrap-new-machine.ps1 -RepoPath <REPO_PATH>
```

## 验收

- `legacy/bootstrap-new-machine.ps1` 返回 `Code intel bootstrap: OK`
- `legacy/check-code-intel-tools.ps1 -RepoPath <REPO_PATH>` 返回 OK
- 最新 artifact 目录里存在：
  - `summary.md`
  - `report.json`
  - `understanding.md`
...
- 先读平台 code-intel data root 下的 `bootstrap/bootstrap-*.md`
- 再读 smoke artifact 里的 `summary.md`
- 如果缺真实 `sentrux`，不用先修；shim 会启用 lite core
