...
fn temp_repo(label: &str) -> TempTree {
    ...
    let root = std::env::temp_dir().join(format!(
        "code-intel-design-proposal-{label}-{}-{nonce}-{sequence}",
        std::process::id()
    ));
...
fn fixture(name: &str) -> Value {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/design-proposal")
        .join(name);
    serde_json::from_slice(&fs::read(path).expect("read proposal fixture"))
        .expect("proposal fixture JSON")
}
...
fn setup(name: &str) -> (TempTree, PathBuf, PathBuf, Value) {
    ...
    (
        ...
        repo,
        out,
        json!({"candidate":candidate_path,"context":context_path}),
    )
}
...
fn valid_two_option_candidate_stages_advisory_result() {
    ...
    let output = run_capability(
        &validate_request(
            &repo,
            &as_path(&paths["context"]),
            &as_path(&paths["candidate"]),
            &out,
        ),
        &tree.0.join("request.json"),
        &out,
    );
    assert!(
        output.status.success(),
    ...
    let artifact_ref =
        published_artifact_ref(&result, "code-intel-design-proposal.v1", "design.proposal");
    let payload = staged_artifact_payload(
        &out,
        artifact_ref,
    ...
    assert_eq!(
        payload["snapshot"]["identity"],
        snapshot_for(&repo)["identity"]
    );
    let ids: Vec<_> = payload["options"]
        .as_array()
        .unwrap()
        .iter()
...
fn context_mode_stages_design_context() {
    ...
    let output = run_capability(
        &context_request(&repo, &out),
        &tree.0.join("context-request.json"),
        &out,
    );
    assert!(
        output.status.success(),
    ...
    assert!(result["artifacts"]
        ...
        .unwrap()
        .iter()
        .any(|a| a["artifactSchema"] == "code-intel-design-context.v1"
            && a["type"] == "design.context"));
}
...
#[test]
fn invalid_recommendation_is_rejected() {
    assert_invalid(
...
fn stale_snapshot_is_rejected() {
    assert_invalid("stale-snapshot.json", "proposal_snapshot_mismatch");
}
#[test]
fn non_identity_snapshot_field_is_rejected() {
    assert_invalid("snapshot-field-mismatch.json", "proposal_snapshot_mismatch");
}
#[test]
fn drifted_evidence_is_rejected() {
    assert_invalid("drifted-evidence.json", "proposal_evidence_drifted");
}
#[test]
fn missing_method_evidence_is_rejected() {
    assert_invalid(
        "missing-method-evidence.json",
        "proposal_method_not_applicable",
    );
}
#[test]
fn authority_escalation_is_rejected() {
    assert_invalid("authority-escalation.json", "proposal_authority_escalation");
}
...
fn option_count_diagnostic_rejects_one_and_four_options() {
    ...
    for count in [1usize, 4] {
        ...
        let output = run_capability(
            &validate_request(&repo, &as_path(&paths["context"]), &candidate_path, &out),
            &tree.0.join(format!("request-{count}.json")),
            &out,
        );
