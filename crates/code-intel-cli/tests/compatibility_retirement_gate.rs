...
fn e01_ticket_is_content_bound_to_the_approved_e00_subject() {
    ...
    let deletion = deletion_diff(&temp.0, &["legacy/run-code-intel.ps1"]);
    ...
    let ticket = json!({
        ...
        "legacyBranch":{"capabilityId":"legacy.capability","branchId":"legacy.branch","callPath":"legacy/run-code-intel.ps1::legacy.branch"},
        "replacement":{"capabilityId":"replacement.atom","dependencies":["D02"]},"affectedFiles":["legacy/run-code-intel.ps1"],
    ...
    let hidden_deletion =
        deletion_diff(&temp.0, &["legacy/run-code-intel.ps1", "second-branch.ps1"]);
    ...
    hidden_ticket["affectedFiles"] = json!(["legacy/run-code-intel.ps1", "second-branch.ps1"]);
